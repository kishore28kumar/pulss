...
interface FormattedReturnPolicyProps {
  text: string;
}

export default function FormattedReturnPolicy({ text }: FormattedReturnPolicyProps) {
  ...
  const parseText = (text: string) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Skip empty lines, but add spacing if previous element wasn't a separator
      if (!trimmed) {
        // Add spacing between sections if needed
        if (prevElement && prevElement.type !== 'separator' && prevElement.type !== 'paragraph') {
          // Don't add extra spacing, let CSS handle it
        }
        continue;
      }
